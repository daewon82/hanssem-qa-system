...
// ─── 설정 ────────────────────────────────────────────────────
const REPORT_ID = "stg-mw-e2e";
const REPORT_TITLE = "Stage MW E2E 테스트";
...
test.describe("Stage MW E2E 테스트", () => {
...
test.beforeAll(async () => {
  ...
  await updateProgress("stg-mw-e2e");
});
...
test.afterAll(async () => {
  ...
  console.log(`🏁 Stage MW E2E 완료! 총 ${totalCount}건`);
});
...
test.describe("6. 로그인", () => {
  test("로그인 페이지 — ID 입력 필드 및 SNS 버튼 노출", async ({ page }) => {
    await page.goto(
      "https://stagemall.hanssem.com/customer/mallLoginMain.do?returnUrl=https://stg-m.store.hanssem.com/"
    );
    await waitForPageReady(page, 3000);
...
test.describe("7. 장바구니", () => {
  test("장바구니 직접 URL — 로그인 redirect 확인", async ({ page }) => {
    await page.goto("https://stagemall.hanssem.com/m/morder/goCart.do");
    await waitForPageReady(page, 2000);
    const url = page.url();
...
test.describe("12. 인테리어 서브 페이지", () => {
  test("카테고리 페이지 진입 및 상품 노출", async ({ page }) => {
    await page.goto("https://stg-m.store.hanssem.com/category/20002");
    await waitForPageReady(page, 3000);
    await expect(page).toHaveTitle(/한샘/);
    const response = await page.request.get("https://stg-m.store.hanssem.com/category/20002");
    expect(response.status()).toBeLessThan(400);
    console.log(`[✓] 카테고리 페이지 로딩: ${page.url()}`);
  });

  test("인테리어 시공사례 페이지 진입", async ({ page }) => {
    await page.goto("https://stg-m.store.hanssem.com/interior/constcase");
    await waitForPageReady(page, 3000);
    await expect(page).toHaveTitle(/한샘/);
    await expect(page.locator("header").first()).toBeVisible();
