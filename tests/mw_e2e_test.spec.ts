 ...
 * 시나리오:
 * 1. 메인 페이지 — 로딩, 헤더, 푸터
 * 2. 카테고리 네비게이션 — 가구/홈리빙, 인테리어
 * 3. 상품 목록 — 상품 링크 노출 확인
 * 4. 상품 상세 — 상품명(H1), 가격, 구매 버튼 노출
 * 5. 검색 — 검색 결과 페이지 진입 및 응답 확인
 * 6. 로그인 — 로그인 버튼 클릭 → mall.hanssem.com 이동
 * 7. 장바구니 — 장바구니 버튼 클릭 → mall.hanssem.com 이동
 * 8. 주요 페이지 HTTP 응답 확인
 * 9. 매장 찾기 — /store 페이지 진입 및 GNB 버튼 클릭
 */
