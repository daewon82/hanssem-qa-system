...
// ─── 설정 ────────────────────────────────────────────────────
const REPORT_ID = "stg-pc-e2e";
const REPORT_TITLE = "Stage PC E2E 테스트";
...
test.beforeAll(async () => {
  ...
  await updateProgress("stg-pc-e2e");
});
...
test.afterAll(async () => {
  ...
  const reportIdx = existingData.stageReports.findIndex((r: any) => r.id === REPORT_ID);
  if (reportIdx >= 0) {
  ...
  fs.writeFileSync(
    "public/stg_pc_e2e.json",
    JSON.stringify({ title: REPORT_TITLE, lastUpdated: kst, total: totalCount, pass: passCount, fail: failCount, passRate, cases: caseResults }, null, 2),
  );
  ...
  console.log(`🏁 Stage PC E2E 완료! 총 ${totalCount}건`);
});
...
// ─── 5. 검색 ───────────────────────────────────────────────
test.describe("5. 검색", () => {
  test("검색 아이콘 클릭 → 검색창 → 결과 이동 (PC)", async ({ page, isMobile }) => {
  ...
  test("검색 결과 페이지 — 정상 로딩", async ({ page }) => {
...
// ─── 6. 로그인 ─────────────────────────────────────────────
test.describe("6. 로그인", () => {
  test("로그인 버튼 클릭 → stagemall.hanssem.com 이동 (PC)", async ({ page, isMobile }) => {
    test.skip(isMobile, "PC GNB 전용");
    await page.goto("/");
    await waitForPageReady(page, 3000);
    ...
    await page.waitForURL(/stagemall\.hanssem\.com/, { timeout: 10000 });
    await expect(page).toHaveURL(/stagemall\.hanssem\.com/);
    console.log(`[✓] 로그인 이동: ${page.url()}`);
  });

  test("로그인 페이지 — ID 입력 필드 및 SNS 버튼 노출", async ({ page }) => {
    await page.goto(
      "https://stagemall.hanssem.com/customer/mallLoginMain.do?returnUrl=https://stg-store.hanssem.com/"
    );
    await waitForPageReady(page, 3000);
    await expect(page).toHaveURL(/stagemall\.hanssem\.com.*login/i);
    const idInput = page.locator('input[type="text"], input[type="email"], input[name*="id"]').first();
    const snsBtn = page.locator('button:has-text("카카오"), a:has-text("카카오")').first();
...
test.describe("7. 장바구니", () => {
  test("장바구니 버튼 클릭 → stagemall 이동 (PC)", async ({ page, isMobile }) => {
    test.skip(isMobile, "PC GNB 전용");
    await page.goto("/");
    await waitForPageReady(page, 3000);
  ...
  test("장바구니 직접 URL — 로그인 redirect 확인", async ({ page }) => {
    const cartUrl = "https://stagemall.hanssem.com/order/goCart.do?util=shopping";
...
test.describe("9. 매장 찾기", () => {
  ...
  test("GNB 매장 찾기 버튼 클릭 → 매장 검색 페이지 이동 (PC)", async ({ page, isMobile }) => {
    ...
    await page.waitForURL(/store|stg-m\.remodeling\.hanssem\.com/, { timeout: 10000 });
    const url = page.url();
    expect(url.includes("/store") || url.includes("stg-m.remodeling.hanssem.com")).toBe(true);
...
test.describe("11. 상품 목록 정렬/필터", () => {
  ...
  test("가구/홈리빙 — 정렬 변경 (낮은가격순)", async ({ page, isMobile }) => {
    test.skip(isMobile, "PC 전용");
    await page.goto("/furnishing");
    await waitForPageReady(page, 4000);
...
test.describe("12. 인테리어 서브 페이지", () => {
  test("카테고리 페이지 진입 및 상품 노출", async ({ page }) => {
    await page.goto("https://stg-store.hanssem.com/category/20002");
    await waitForPageReady(page, 3000);
    await expect(page).toHaveTitle(/한샘/);
    const response = await page.request.get("https://stg-store.hanssem.com/category/20002");
    expect(response.status()).toBeLessThan(400);
    console.log(`[✓] 카테고리 페이지 로딩: ${page.url()}`);
  });

  test("인테리어 시공사례 페이지 진입", async ({ page }) => {
    await page.goto("https://stg-store.hanssem.com/interior/constcase");
    await waitForPageReady(page, 3000);
    await expect(page).toHaveTitle(/한샘/);
    await expect(page.locator("header").first()).toBeVisible();
  ...
  test("인테리어 — 무료견적상담 링크 노출", async ({ page }) => {
    ...
    const consultLink = page
      ...
      .first();
    await expect(consultLink).toHaveAttribute("href", /stg-m\.remodeling\.hanssem\.com/, { timeout: 8000 });
    console.log("[✓] 무료견적상담 링크 확인");
  });

  test("기획전 페이지 200 응답", async ({ page }) => {
    const response = await page.goto("https://stagemall.hanssem.com/plan/19523.html");
    expect(response?.status()).toBeLessThan(400);
    await expect(page).toHaveTitle(/한샘/);
