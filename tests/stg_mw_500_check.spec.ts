...
// [설정]
const TARGET_DOMAIN = "https://stg-m.store.hanssem.com";
const MAX_LINKS = 50;
...
test("Stage MW 랜딩 테스트", async ({ page }, testInfo) => {
  test.setTimeout(7200000);
  await updateProgress("stg-mw-landing", 0, MAX_LINKS);
      ...
      Array.from(document.querySelectorAll("a"))
        ...
        .filter(
          (h) =>
            h &&
            h.startsWith("http") &&
            h.includes("stg-m.store.hanssem.com") &&
            !h.includes("#"),
        ),
    );
  ...
  while (visitedLinks.size < MAX_LINKS) {
    ...
    if (visitedLinks.size % 10 === 0 || visitedLinks.size === MAX_LINKS) {
      console.log(`[PROGRESS] STG_MW_500: ${visitedLinks.size}/${MAX_LINKS}`);
    }

    if (Date.now() - lastProgressUpdate > 5000) {
      await updateProgress("stg-mw-landing", visitedLinks.size, MAX_LINKS);
  ...
  const reportIdx = existingData.stageReports.findIndex((r: any) => r.id === "stg-mw-landing");
  if (reportIdx >= 0) {
  ...
  fs.writeFileSync(
    "public/stg_mw_500.json",
    JSON.stringify({ title: "Stage MW 500개 랜딩 테스트", lastUpdated: kst, total: totalCount, pass: passCount, fail: failCount, passRate, cases: caseResults }, null, 2),
  );

  await publishResults(
    newReport,
    { title: "Stage MW 500개 랜딩 테스트", lastUpdated: kst, total: totalCount, pass: passCount, fail: failCount, passRate, cases: caseResults },
    "stg_mw_500.json"
  );

  console.log(`🏁 Stage MW 점검 완료! 총 ${totalCount}건 확인.`);
});
